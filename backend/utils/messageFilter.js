...
// Enterprise Multi-Layer Contact & Social-Media Leak-Proof Filter
// Permits all legitimate educational content (Maths, Physics, Chem, Class 10, ₹500/hr)
// Strictly detects & blocks disguised phone numbers, social handles, emails & links
// ============================================================
...
const normalizeText = (text) => {
  ...
  return text
    ...
    // Normalize emoji numbers (0️⃣ - 9️⃣)
...
const stripEducationalContext = (text) => {
  return text
    ...
    .replace(/\b\d{1,3}\s*%/g, ' ')
    .replace(/\b\d{1,3}\s*\/\s*\d{1,3}\b/g, ' ') // e.g. 95/100
    // Time / Timings
...
const detectPhoneNumbers = (rawText) => {
  ...
  // Match any digit sequence with optional separators (spaces, dots, hyphens, brackets, symbols)
  // e.g. "9876543210", "98765 43210", "+91 98765-43210", "9.8.7.6.5.4.3.2.1.0", "9@8#7$6%5^4&3*2(1)0"
  ...
  if (potentialMatches) {
    for (const match of potentialMatches) {
      ...
      // Country code +91 (12 digits)
      if (/^91[6-9]\d{9}$/.test(digitsOnly)) return true;
      // Leading zero (11 digits)
      if (/^0[6-9]\d{9}$/.test(digitsOnly)) return true;
      // Any generic phone sequence with 8 or more contiguous/spread digits
      if (digitsOnly.length >= 8) return true;
    }
  }

  // Check leet-speak transformed digits (e.g. 98765432lO with l=1, O=0)
...
const detectSpelledOutNumbers = (text) => {
  ...
  // Split into tokens by punctuation, spaces, and special symbols
  ...
  for (const token of tokens) {
    if (NUMBER_WORD_MAP[token] !== undefined || ROMAN_DIGIT_MAP[token] !== undefined) {
      sequenceCount++;
      if (sequenceCount >= 5) return true; // 5+ number words in sequence indicates a phone number
    } else {
      sequenceCount = 0;
    }
  }

  // Check compressed number words (e.g. "nineeightsevensixfivefourthreetwoonezero")
...
const detectEmails = (rawText) => {
  ...
  // 2. Disguised Email: name [at] domain [dot] com / name (at) domain (dot) com
...
const detectSocialMediaHandles = (rawText) => {
  ...
  // 1. Instagram variations: "insta:", "ig @", "i.g. /", "@username", "instagram: username"
  ...
  // 2. Snapchat variations: "snap:", "snapchat id:", "sc: username", "snap me at"
  ...
  // 3. Telegram variations: "tg:", "telegram:", "t.me/", "telegram @"
  ...
  // 4. WhatsApp variations: "wa:", "whatsapp:", "ping on wa", "msg on whatsapp"
  ...
  for (const phrase of directContactPhrases) {
    if (normalized.includes(phrase)) {
      // If accompanied by any handle, number, or social keyword
...
const detectExternalLinks = (rawText) => {
  ...
  // Disguised domain (e.g. "telegram dot me slash username" or "wa dot me slash")
...
/**
 * Master contact-information detector: Combines all layers
 * @param {string} text
 * @returns {boolean} true if ANY contact sharing or external channel is detected
 */
const hasContactInfo = (text) => {
  ...
  if (detectPhoneNumbers(raw)) return true;

  // 2. Spelled out numbers (English, Hindi Latin, Hindi Devanagari, Roman numerals)
  if (detectSpelledOutNumbers(raw)) return true;

  // 3. Emails (standard, disguised, bracketed)
  if (detectEmails(raw)) return true;

  // 4. Social media usernames, profile handles & direct contact requests
  if (detectSocialMediaHandles(raw)) return true;

  // 5. External contact links & shorteners
  if (detectExternalLinks(raw)) return true;
